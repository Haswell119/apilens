...
(function (global) {
  ...
  APILens.maxNodes = APILens.maxNodes || 20000;
  ...
  function renderNode(parent, key, value, depth, counter) {
    if (counter.n >= (APILens.maxNodes || 20000)) { counter.truncated++; return; }
    counter.n++;
  ...
  APILens.render = function (container, text, type) {
    ...
    if (counter.truncated > 0) {
      wrap.appendChild(el('div', 'apl-truncated',
        'Truncated at ' + (APILens.maxNodes || 20000) + ' nodes — ' + counter.truncated + ' more hidden. Pro unlocks large-file mode.'));
    }
